...
当你看到这里的时候, 现行的 Docker 镜像规范已经完全介绍完毕, 从下一篇文章开始就进入**实战**内容。预期在下一章里, 我会为大家介绍如何**徒手构建一个 Docker 简单的镜像, 再借助 runc 从该镜像创建容器实例**, 以进一步探讨镜像中各 `Layer` 中记录的 `Filesystem Changeset` 的内容, 为最后介绍如何构建镜像打下铺垫。
